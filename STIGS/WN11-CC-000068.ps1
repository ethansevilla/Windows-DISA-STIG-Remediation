<#
.SYNOPSIS
    This PowerShell script enables delegation of non-exportable credentials
    for remote hosts on Windows 11.
...
try {
    ...
    # Create or update AllowProtectedCreds as REG_DWORD 1.
    New-ItemProperty `
        -Path $RegistryPath `
        -Name $ValueName `
        -PropertyType DWord `
    ...
    if ($CurrentValue -eq $RequiredValue) {
        Write-Host ""
        Write-Host "WN11-CC-000068 remediation completed successfully."
    ...
    Write-Error "WN11-CC-000068 remediation failed: $($_.Exception.Message)"
    exit 1
}
