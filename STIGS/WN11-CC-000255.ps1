<#
.SYNOPSIS
    This PowerShell script requires the use of a hardware security device,
    such as a Trusted Platform Module (TPM), for Windows Hello for Business
    on Windows 11.
...
try {
    ...
    # Create or update RequireSecurityDevice as REG_DWORD 1.
    New-ItemProperty `
        -Path $RegistryPath `
        -Name $ValueName `
        -PropertyType DWord `
    ...
    if ($CurrentValue -eq $RequiredValue) {
        Write-Host ""
        Write-Host "WN11-CC-000255 remediation completed successfully."
    ...
    Write-Error "WN11-CC-000255 remediation failed: $($_.Exception.Message)"
    exit 1
}
