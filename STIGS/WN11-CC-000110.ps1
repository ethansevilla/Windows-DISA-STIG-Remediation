...
.SYNOPSIS
    ...
    by enabling the DisableHTTPPrinting policy setting.
...
try {
    ...
    # Create or update DisableHTTPPrinting as a REG_DWORD value of 1.
    New-ItemProperty `
        -Path $RegistryPath `
        -Name $ValueName `
        -PropertyType DWord `
    ...
    if ($CurrentValue -eq $RequiredValue) {
        Write-Host ""
        Write-Host "WN11-CC-000110 remediation completed successfully."
    ...
    Write-Error "WN11-CC-000110 remediation failed: $($_.Exception.Message)"
    exit 1
}
